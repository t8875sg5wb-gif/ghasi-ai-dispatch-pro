...
export const Route = createFileRoute("/buchhaltung")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Einnahmen, Ausgaben, Kostenstellen und betriebswirtschaftliche Auswertung." },
    ],
  }),
...
function BuchhaltungPage() {
  ...
  const kostenpositionen = (Object.keys(KOSTEN_LABEL) as (keyof typeof KOSTEN_LABEL)[]).map((k) => ({
    key: k,
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      <section className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        ...
        <StatCard label="Ausgaben (Monat)" value={EUR(kpis.ausgabenMonat)} icon={TrendingDown} tone="warning" />
        <StatCard label="Gewinn (Monat)" value={EUR(kpis.gewinnMonat)} icon={TrendingUp} tone="success" hint={`Marge ${kpis.margeProzent} %`} />
        <StatCard label="Offene Posten" value={EUR(kpis.offenePosten)} icon={Wallet} tone="info" hint={`${kpis.anzahlOffen} Rechnungen`} />
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
              ...
              <div key={p.key}>
                <div className="mb-1.5 flex items-center justify-between text-sm">
                  ...
                  <span className="font-semibold tabular-nums">
                    {EUR(p.wert)} <span className="text-xs text-muted-foreground">· {p.anteil} %</span>
                  </span>
                </div>
        ...
        <Card className="border-border/70 shadow-card">
          ...
          <CardContent className="space-y-4">
            <div>
              <p className="text-3xl font-bold tabular-nums text-success">{EUR(kpis.gewinnMonat)}</p>
            ...
            <div className="space-y-2 text-sm">
              ...
              <Row label="Ausgaben" value={`− ${EUR(kpis.ausgabenMonat)}`} tone="text-destructive" />
      ...
      <Card className="border-border/70 shadow-card">
        ...
        <CardContent className="space-y-2.5">
          {offen.length === 0 && <p className="text-sm text-muted-foreground">Keine offenen Posten.</p>}
          {offen.map((o) => (
            <div key={o.kundeId} className="flex items-center justify-between rounded-xl border border-border/60 bg-muted/30 p-3">

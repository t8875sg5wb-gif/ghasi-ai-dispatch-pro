...
test("insurer", () => {
  expect(
    insurerFieldsSchema.safeParse({ name: "AOK", kuerzel: "AOK", vertragsstatus: "Rahmenvertrag" })
      .success,
  ).toBe(true);
  expect(
    insurerFieldsSchema.safeParse({ name: "AOK", kuerzel: "AOK", vertragsstatus: "Einzelvertrag" })
      .success,
  ).toBe(false);
  expect(
    insurerFieldsSchema.safeParse({
      name: "AOK",
      kuerzel: "AOK",
      vertragsstatus: "Einzelfall",
      x: 1,
    }).success,
  ).toBe(false);
});
test("contract", () => {
  ...
  expect(insurerContractFieldsSchema.safeParse({ ...ok, gueltigAb: "01.01.2026" }).success).toBe(
    false,
  );
});
test("expense", () => {
...
test("call", () => {
  ...
  expect(
    callFieldsSchema.safeParse({ ...ok, zeitpunkt: "2026-07-04T02:00:11.057+00:00" }).success,
  ).toBe(true);
